...
哪些**只有 AHU-601 為據**，正式設備上尚待驗證：

  · 量程 ±4g 與由此換算的 FULL_SCALE_MS2（僅 SENSOR_SATURATION 用到）
  · 加速度的絕對單位 m/s²
  · 滾動窗口 10 秒（目前僅用於推理獨立樣本數，程式未直接使用）

實務影響有限但不是零：泵浦的 accPEAK 只到滿刻度的 2~7%（AHU 曾達 56%），
離飽和很遠，所以 SENSOR_SATURATION 目前不會因為量程猜錯而誤判；但若正式
設備的量程不同，該規則的門檻就是錯的。要收斂這件事，需要一份**泵浦的
rawdata**（三軸原始值），比照 AHU 的方式重新反推。
"""
