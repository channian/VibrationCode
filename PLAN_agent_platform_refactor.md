...
### 🔄 感測器方向：重力法可行但不適用於例行監測

實測證實 raw 的直流分量即重力向量，可定出感測器姿態（本樣本重力單位向量 x = −0.433、y = +0.901、z = +0.030 → y 為垂直方向，z 位於水平面內，x 有 43% 分量代表安裝面傾斜）。

**但 raw 需工程師現場量測、無自動化排程**（見 §一），因此重力法**不納入例行監測**，僅列為現場量測時可順帶取得的資訊。

例行監測的方向處理維持 §五 的設計：
- 分析一律使用合成值與**排序後**的軸能量佔比 → 與座標方向無關，直接迴避貼錯問題
- `ORIENTATION_CHANGE` 規則以**軸能量分佈排列的跳變**判定（非重力法），可偵測感測器重貼或更換
- 若工程師現場量測時取得 raw，可順帶算出重力方向作為安裝品質的佐證
...
| 項目 | 決定 |
|------|------|
| 資料庫 | **PostgreSQL**，DDL 見 `db/schema.sql`（已實測建置通過） |
| 使用者認證 | **AD 登入**；`app_user` 以 `ad_username` + `ad_sid` 對應，SID 為不變識別碼避免帳號改名失聯 |
| 角色管理 | 走 DB（`app_role` / `user_role`），網頁同步維護；支援依廠區／樓層／系統別限定範圍 |
| SLA | **各階段 5 天**，已寫入 `sla_config`，可由管理員調整 |
...
2. 電流 DB 的種類與連線方式（Sensor 端 Phase 1 走檔案，暫不需要）
3. RAG 語料現況（簽核鏈上線後會自行累積，初期可先只接技術文獻）
4. Dashboard 細節 — 見另份《Dashboard 需求書》待確認事項
5. 部署環境與 Python 版本
6. API Key 發放管理方式
