...
### 3.0 若歷史資料橫跨前端改版，先加 `--latest-cadence-only`

前端有兩種輸出版本：**即時量測每秒一筆、長期量測每 10 分鐘一筆**。設備是
陸續換版的，所以一份匯出檔裡常同時存在兩種密度，而且各設備的換版日期不同。

混雜密度會同時造成三個問題：換版空窗被算成感測器離線、涵蓋率分母被撐大、
以及基準期偏誤——每小時樣本數多的區段變異天生較小，會贏得「最穩定窗口」
的選擇，再拿去比對低密度資料就整段看起來都在偏離（實測 STEP_CHANGE 誤報
率因此從 8% 升到 100%）。基準期偏誤已在程式內擋掉，前兩者則需要裁切：

```bash
python -m validate.offline --data-dir <資料夾> --latest-cadence-only
```

**逐點各自裁切，不要自己統一切一個日期。** 實測同一批資料裡有 3/31、5/18、
6/4 等多個換版日，統一切在 6/1 會讓 3 月就換好版的設備從 142 天的可用資料
縮到 80 天。真的需要指定期間時用 `--since` / `--until`。
