...
- **`--device-meta` 台帳檔**（強烈建議補，不是可有可無的選項）。Analytic
  ...
  **退回 σ 的門檻遠比 ISO 錨定門檻敏感**：66 台 11 週（2026-06-05 起，
  已排除換版斷層）的實測，同一批資料跑兩輪，唯一差別是有沒有帶台帳——
  台帳空白時工程師每週要處理 10.6 件，填了之後是 3.8 件
  （VEL_HIGH 117 次 / 36 台 vs 41 次 / 10 台）。不填台帳省下的是填兩個
  欄位的功夫，換來的是 **2.8 倍**的工作量。

  （早期文件引用的「4.3 倍」來自 33 週那一輪，當時窗口含一月換版斷層、
  涵蓋率僅 51.1%，資料稀疏會抬高未分類那一輪的 σ 觸發量而放大差距。
  請以乾淨窗口的 2.8 倍為準，完整對照見
  `docs/DECISIONS_2026-09_expert_review.md` 的 D1。）
