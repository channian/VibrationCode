...
#: 每個待填欄位的完整說明：(欄位, 填什麼, 為什麼要填, 怎麼判斷)。
#: 這份說明會跟 CSV 一起輸出成檔案，讓工程師手上有東西可看——
#: 只給一張空白表格而沒有「這欄為什麼重要」，填出來的品質會差很多，
#: 而且填錯了沒人知道（ISO 分類填錯不會報錯，只會讓判定安靜地偏掉）。
_LEDGER_FIELD_GUIDE = [
    ('rated_power_kw', '額定功率，單位 kW。銘牌上的數字，例如 22、45、75。',
     'ISO 20816-3 只適用於 15 kW 以上的機器。沒有這欄，系統無法判斷哪些設備'
     '本來就不在標準範圍內，會對不該套用的設備硬套 Zone 判定。',
     '看馬達銘牌。同型號可整批填。真的查不到就留空——留空是「不知道」，'
     '系統會跳過範圍檢查，比填一個猜的數字好。'),
    ('iso_machine_group', "ISO 10816-3 的機器群組，填 1、2、3 或 4。",
     '這一欄與基礎剛性共同決定 Zone A/B/C/D 的邊界數值。兩者缺一，'
     '整台設備就無法做絕對位準判定，只能退回「跟自己比」。',
     'Group 1：大型機（額定 300 kW 以上，或軸高 315 mm 以上）。'
     'Group 2：中型機（15–300 kW，或軸高 160–315 mm）。'
     'Group 3：泵浦，葉輪與驅動分離（外接馬達，用聯軸器連接）。'
     'Group 4：泵浦，葉輪與驅動整合（同一軸，例如直結式、沉水式）。'
     '本廠多數冰水泵與冷卻水泵落在 Group 3 或 4，差別只在驅動型式；'
     '空調箱風機多半是 Group 2。'),
    ('iso_foundation', "基礎剛性，填 rigid（剛性）或 flexible（柔性）。",
     '**同一個群組下，剛性與柔性的 Zone 邊界可以差將近一倍**'
     '（例如 Group 2：剛性 A/B 界 1.40 mm/s，柔性 2.30 mm/s）。'
     '填錯會讓判定整個偏掉，而且不會有任何錯誤訊息。',
     '判準是機器與基礎的第一階自然頻率關係：機組本身的自然頻率明顯高於'
     '運轉轉速頻率 → rigid；低於運轉轉速頻率 → flexible。'
     '實務上的簡易判斷：直接以地腳螺栓鎖固在混凝土基座或厚鋼板上 → rigid；'
     '裝在防震墊、彈簧、避震器上，或裝在樓板、鋼構平台上 → flexible。'
     '同型號、同安裝方式可整批填。'),
    ('iso_driver_type', "驅動型式，填 integrated（整合）或 external（外接）。選填。",
     '僅供台帳稽核與日後回查，Zone 判定不從這裡推導'
     '（推導規則本身需要專家確認，不該埋在程式裡）。',
     '葉輪與馬達同軸 → integrated；用聯軸器連接分離的馬達 → external。'
     '不確定就留空，不影響判定。'),
    ('is_standby', '備機旗標，填 TRUE 或 FALSE。',
     '備機一天只跑一兩小時，湊不到建立基準期所需的資料量，'
     '所有「跟自己比」的規則會安靜地不觸發。標記出來系統才知道'
     '這是正常狀態而不是故障，也才能對備機套用不同的監測策略。',
     '常態運轉的主機填 FALSE，輪值待機的備機填 TRUE。'
     '**不確定就留空**——留空代表「不知道」，不會覆蓋台帳既有的設定；'
     '填 FALSE 則是「已確認不是備機」，兩者意義不同。'),
    ('last_maintenance_at', '最後一次大修／換軸承／基礎變更的日期，格式 YYYY-MM-DD。選填。',
     'ISO 10816-3 §5.4.1 要求大修後重新檢討基準值。基準期是程式掃描'
     '「最穩定的一段時間」選出來的，而穩定不等於健康——機器大修後振動下降，'
     '若基準仍停留在大修前，系統會把「變好」判讀成偏離。',
     '填了之後，基準期不會早於這一天。只需要填最近一次；'
     '沒做過大修或查不到就留空。'),
]


def write_ledger_guide(csv_path: str) -> str:
    """把欄位說明寫成 CSV 旁邊的一份文件，連同範本一起交給工程師。"""
    path = os.path.splitext(csv_path)[0] + '_填寫說明.md'
    lines = [
        '# 振動監測台帳　填寫說明',
        '',
        f'對應檔案：`{os.path.basename(csv_path)}`',
        '',
        '## 這份資料在做什麼',
        '',
        '振動感測器的分析檔只有量測數值，沒有「這台機器是什麼」的資訊。',
        'ISO 10816-3 判斷振動大小是否可接受時，門檻會隨機器類型與安裝方式變動，',
        '所以要先知道每台設備的分類，才能套用正確的門檻。',
        '',
        '沒有這份台帳，系統只能拿每台設備「跟自己的過去」比，',
        '無法回答「這個振動水準在工程上算不算可接受」。',
        '實測差距：填了台帳之後需要人工處理的告警量，約為不填的四分之一。',
        '',
        '## 填寫原則',
        '',
        '1. **`iso_machine_group` 與 `iso_foundation` 必須成對填。**',
        '   只填一邊算不出 Zone，系統會把整台當成未分類，等於白填。',
        '2. **同型號、同安裝方式的設備可以整批填同一組值**，不必逐台勘查。',
        '3. **不確定就留空，不要猜。** 留空代表「不知道」，系統會跳過該項判定；',
        '   填一個錯的值則會讓判定安靜地偏掉，沒有任何錯誤訊息。',
        '4. **不必一次填到完美。** 系統有一條回頭檢查的規則：當某台設備的實測',
        '   振動水準與所填分類明顯矛盾時會告警，可以據此修正。',
        '5. 前幾欄（設備代碼、轉速、運轉樣本數、velRMS 中位數與 p95）是系統',
        '   預填的參考資料，**請不要修改**。轉速與振動水準可以幫助判斷分類。',
        '',
        '## 欄位說明',
        '',
    ]
    for field, what, why, how in _LEDGER_FIELD_GUIDE:
        lines += [
            f'### `{field}`',
            '',
            f'**填什麼**：{what}',
    ...
    lines += [
        '## 填完之後',
        '',
        '存成 CSV（保持 UTF-8 編碼）交回即可，不需要轉檔或改欄位名稱。',
        '',
    ]
    os.makedirs(os.path.dirname(os.path.abspath(path)) or '.', exist_ok=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.emit_ledger:
        ...
        guide_path = write_ledger_guide(args.emit_ledger)
        report_ledger_template(args.emit_ledger, n)
        print(f'欄位填寫說明（可直接轉給工程師）：{guide_path}')
        return 0
